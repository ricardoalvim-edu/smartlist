...
<html>
    ...
    <body style="max-width: 80%; margin: 0px auto; margin-top: 4%;">
        <h1>Todos os produtos do ${sessionScope['estabelecimento']}</h1>
        <table id="example" class="display display" cellspacing="0" width="100%">
            ...
            <tbody>
                <c:if test="${empty produto.ListByEstabelecimento(sessionScope['estabelecimentoId'])}">
                    <h1>Não existem produtos ainda :(</h1>
                </c:if>
                    <c:forEach var="produtos" items="${produto.ListByEstabelecimento(sessionScope['estabelecimentoId'])}">
                        <tr>
                            <td>
